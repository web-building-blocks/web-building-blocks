...
Give the input the correct type. For example, if it is email input, the type should be email. We cannot let the user type in their birthday but the type we assigned is email.
